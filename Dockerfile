...
RUN if [[ ! -d vendor ]]; then echo "No vendor directory found. Run ``make vendor`` to create it"; false; fi
RUN apk --no-cache add git runit@community
RUN go build -o kubeac &&\
