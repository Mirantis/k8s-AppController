...
func enqueueTasks(added, deleted <-chan *v1.ConfigMap, c client.Interface) {
	...
	listSync := &sync.Mutex{}
	go deployTasks(taskQueue, listSync, cond, c)
	for {
		select {
		case add := <-added:
			listSync.Lock()
...
func deployTasks(taskQueue *list.List, mutex *sync.Mutex, cond *sync.Cond, c client.Interface) {
	...
	for {
		...
		for {
			mutex.Lock()
			back = taskQueue.Back()
			processingNodeWasRemoved := processing != nil && taskQueue.Front() != processing && processing.Prev() == nil && processing.Next() == nil
			mutex.Unlock()
