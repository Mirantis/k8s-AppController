...
func enqueueTasks(added, deleted <-chan *v1.ConfigMap, c client.Interface) {
	queue := list.New()
	cond := sync.NewCond(&sync.Mutex{})
	listSync := &sync.Mutex{}
	go deployTasks(queue, listSync, cond, c)
	for {
		select {
		case add := <-added:
			listSync.Lock()
			...
			if node == nil {
				...
				for node != nil {
					v := node.Value.(*v1.ConfigMap)
					if isBefore(v, add) {
						queue.InsertBefore(add, node)
						break
					}
					node = node.Next()
				}
		...
		case del := <-deleted:
			signal := false
			listSync.Lock()
			for n := queue.Front(); n != nil && !signal; n = n.Next() {
...
func deployTasks(queue *list.List, mutex *sync.Mutex, cond *sync.Cond, c client.Interface) {
	...
	for {
		...
		for {
			mutex.Lock()
			back = queue.Back()
			processingNodeWasRemoved := processing != nil && queue.Front() != processing && processing.Prev() == nil && processing.Next() == nil
			mutex.Unlock()
		...
		go func(ch <-chan struct{}, el *v1.ConfigMap) {
			defer func() {
				processing = nil
				c.ConfigMaps().Delete(el.Name, nil)
				cond.Signal()
			}()

			log.Println("Starting deployment task", el.Name)
			sched, opts, err := FromConfig(c, el.Data)
			if err != nil {
				log.Println(err)
				return
			}
