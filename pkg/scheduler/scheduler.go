...
func (sr *ScheduledResource) Wait(checkInterval time.Duration, stopChan <-chan struct{}) error {
			...
			for attemptNo := 1; attemptNo <= attempts; attemptNo++ {
				...
				for _, req := range r.RequiredBy {
					go func(req *ScheduledResource, toCreate chan *ScheduledResource) {
						ticker := time.NewTicker(CheckInterval)
						log.Printf("Requesting creation of dependncy %v", req.Key())
						for {
							select {
							case <-stopChan:
				...
				err = r.Wait(CheckInterval, stopChan)
