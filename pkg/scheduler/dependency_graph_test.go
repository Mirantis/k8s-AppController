...
func TestAllocateReplicas(t *testing.T) {
	...
	newReplicas1, deleteReplicas, _ := sched.allocateReplicas(flow, interfaces.DependencyGraphOptions{ReplicaCount: 3})
	...
	newReplicas2, deleteReplicas, _ := sched.allocateReplicas(flow, interfaces.DependencyGraphOptions{ReplicaCount: 1})
	...
	allReplicas, deleteReplicas, _ := sched.allocateReplicas(flow, interfaces.DependencyGraphOptions{
		ReplicaCount:          5,
	...
	allReplicas2, deleteReplicas, _ := sched.allocateReplicas(flow, interfaces.DependencyGraphOptions{ReplicaCount: 0})
...
func TestDeallocateReplicas(t *testing.T) {
	...
	c := mocks.NewClient()
	sched := New(c, nil, 0).(*Scheduler)
	newReplicas1, deleteReplicas1, _ := sched.allocateReplicas(flow, interfaces.DependencyGraphOptions{ReplicaCount: 5})
	...
	newReplicas2, deleteReplicas2, _ := sched.allocateReplicas(flow, interfaces.DependencyGraphOptions{ReplicaCount: -2})
