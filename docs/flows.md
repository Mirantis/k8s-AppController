...
2. **Scope**. This is a label selector than can be applied to dependencies. All graph vertexes, reachable from the 
   ...
   what is going to be deployed for each flow replica. Destruction scope defines, what need to be performed before
...
new entities into this picture. Instead, flow implemented as a yet another resource type that can be used as a graph
...
vertex will be replaced with the subgraph, it stands for.

### Flow resource

Blow is a sample Flow resource, that has all possible attributes:
...
Flows may consume other flows by making dependencies of the consumed flow resource. Moreover, that might be
other resources in the consuming flow that depends on the flow, being consumed. In this case consumed flow is
going to be a parent in both dependencies that form the flow and those, that are part of consuming flow. So it
...
All non-flow resources that are not depend on anything are automatically become dependent on `flow/DEFAULT` flow.
...
the flow must be created explicitly.
...
deleted. However, if both `construction` and `destruction` scopes are present the later will have priority over
...
case they all merged into a single dictionary. If the argument is passed twice with different values, the result
